...
Set `DUB_API_KEY` and `DUB_DEFAULT_URL` on the bot host to enable creator attribution. Links are created only through `/issue-link` inside a creator's private channel (or `#bot-tests`); `url:` overrides the default destination. The background job only refreshes totals for links already issued through Discord and never allocates or posts links automatically. The manager portal reads those source-backed snapshots from Postgres. For Dub partner attribution, pass that creator's Dub `partner_id` (or set `DUB_DEFAULT_PARTNER_ID`). `/delete-link link:<id-or-url> confirm:true` removes a saved link from Dub.
