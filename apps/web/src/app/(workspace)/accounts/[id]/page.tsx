...
import { AccountMatchControl } from "@/components/creator-actions";
...
export default async function AccountDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="page-stack creator-profile">
    ...
    <div className="profile-grid"><div className="profile-main"><section className="panel"><div className="panel-header"><div><h2>Tracked videos</h2><p>Durable Viral snapshots for this account</p></div></div><VideoTable videos={accountVideos} /></section></div><aside className="profile-aside"><section className="panel"><div className="panel-header"><div><h2>Canonical creator</h2><p>One human owns this account</p></div></div>{creator ? <div className="integration-card-body"><UserRoundCheck /><strong>{creator.displayName}</strong><p>{creator.discord.username ? `Discord @${creator.discord.username}` : "Discord identity not reconciled"}</p><Link href={`/creators/${creator.id}`} className="secondary-button">Open Creator 360</Link>{account.linkState === "suggested" ? <small>This is an exact-match suggestion. Confirm it below before its metrics enter creator totals.</small> : null}</div> : <div className="empty-state small"><Link2 /><strong>No creator confirmed</strong><p>Choose the existing Result creator. This will not create a duplicate.</p></div>}<AccountMatchControl accountId={account.id} creators={resultCreators} currentCreatorId={creator?.id ?? null} /></section><section className="panel"><div className="panel-header"><div><h2>Source identity</h2><p>Identifiers used for safe joins</p></div></div><dl className="details-grid"><div className="wide"><dt>Viral organization account</dt><dd>{account.id}</dd></div><div className="wide"><dt>Platform account</dt><dd>{account.platformAccountId}</dd></div><div><dt>Last source refresh</dt><dd>{timeAgo(account.refreshedAt)}</dd></div><div><dt>Latest post</dt><dd>{timeAgo(account.latestPostAt)}</dd></div></dl></section></aside></div>
  </div>;
}
