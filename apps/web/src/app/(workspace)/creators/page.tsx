...
export default async function CreatorsPage() {
  ...
  return <div className="page-stack"><PageTitle eyebrow="CREATOR 360" title="Creators" description="One canonical profile for access, signing, social output, manager context, and history." actions={<button className="primary-button"><Plus /> Add creator</button>} />{data.sourceMode === "live_provider" ? <div className="source-banner compact"><UserRoundSearch /><div><strong>{data.creators.length} creator candidates found from tracked accounts</strong><span>Confirm each match after Neon is connected. Result will not infer signing or Discord state from a username.</span></div></div> : null}<Suspense fallback={<div className="skeleton skeleton-panel" />}><CreatorRoster creators={data.creators} /></Suspense></div>;
}
